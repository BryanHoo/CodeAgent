...
import { taskSnapshot, taskSnapshotResponse } from "./mock-data.js";
import { isRequestRecord, parseRequestRecord } from "./mock-request.js";
import type { AppShellApiState, FixtureQueuedSubmission } from "./mock-state.js";
...
export async function handleAppShellTaskRoute(
  ...
  const turnMatch = /^\/v1\/projects\/([^/]+)\/tasks\/([^/]+)\/turns$/u.exec(url.pathname);
  ...
  if (queueMatch !== null) {
    ...
    const task = state.routedTasks.find((item) => item.id === taskId);
    if (task !== undefined) state.archivedTasks.unshift(task);
    ...
    state.routedTasks = state.routedTasks.filter((item) => item.id !== taskId);
    state.turnsByTask.delete(taskId);
    body = { status: "deleted", taskId };
  } else if (turnMatch !== null && route.request().method() === "POST") {
    const taskId = turnMatch[2] ?? "";
    const request = parseRequestRecord(route.request().postData());
    const input = request["input"];
    if (!isRequestRecord(input) || typeof input["text"] !== "string") {
      throw new Error("Invalid turn request");
    }
    const turnNumber = (state.turnsByTask.get(taskId)?.length ?? 0) + 1;
    const turn = {
      completedAt: "2026-08-27T12:00:02.000Z",
      error: null,
      id: `mock-turn-${String(turnNumber)}`,
      items: [
        {
          id: `mock-user-${String(turnNumber)}`,
          role: "user" as const,
          text: input["text"],
          type: "message" as const,
        },
        {
          id: `mock-assistant-${String(turnNumber)}`,
          role: "assistant" as const,
          text: `Mock response: ${input["text"]}`,
          type: "message" as const,
        },
      ],
    ...
    state.turnsByTask.set(taskId, [...(state.turnsByTask.get(taskId) ?? []), turn]);
    const task = state.routedTasks.find((item) => item.id === taskId);
    if (task !== undefined) {
      task.updatedAt = turn.completedAt;
    }
    body = {
    ...
    const task = {
      id: `task-${String(state.routedTasks.length + 1)}`,
      pinned: false,
      projectId,
      title: "New task",
      updatedAt: "2026-08-27T12:00:00.000Z",
    };
    state.routedTasks = [task, ...state.routedTasks];
    state.turnsByTask.set(task.id, []);
    state.taskSettings.set(`${projectId}:${task.id}`, taskSnapshot.settings);
    body = { task };
  } else if (url.pathname.startsWith("/v1/projects/") && url.pathname.endsWith("/tasks")) {
    const projectId = url.pathname.split("/")[3];
    const sourceTasks =
      url.searchParams.get("archived") === "true" ? state.archivedTasks : state.routedTasks;
    const projectTasks = sourceTasks.filter(
      (task) =>
    ...
    body = {
      ...
      snapshot: {
        ...
        turns: state.turnsByTask.get("task-1") ?? taskSnapshot.turns,
      },
    };
    ...
    body = {
      ...
      snapshot: {
        ...
        title: "续接任务",
        turns: state.turnsByTask.get("task-2") ?? [],
      },
    };
    ...
    body = {
      ...
      snapshot: {
        ...
        plan: null,
        pendingRequests: [],
        settings: state.taskSettings.get(`${projectId}:${taskId}`) ?? taskSnapshot.settings,
        status: "idle",
        turns: state.turnsByTask.get(taskId) ?? [],
