export const conversation = {
  liveReviewer: {
    applied: "审核方已更新，将用于当前回合后续步骤；已有审批与沙箱权限不变",
    targetUnavailable: "目标回合已不可更新，设置已保存供后续回合使用",
  },
  asyncQuestions: {
    pending: "待回答问题", pendingCount: "待回答 · {{count}} 组",
    previous: "上一组问题", next: "下一组问题", collapse: "收起问题", expand: "展开问题",
    other: "其他回答", answer: "回答：{{question}}", send: "发送回答", sent: "已发送",
    failed: "回答未发送，请重试", dismiss: "关闭问题",
  },
  agentComponents: {
    addFile: "添加文件",
    addImage: "添加图片",
    addImageOrFile: "添加图片或文件",
    approval: "审批",
    approvalAccepted: "请求已允许",
    approvalRejected: "请求已拒绝",
    approvalRequest: "{{title}}请求",
    codeComments: "{{count}} 个代码评论",
    contextPercent: "上下文已使用 {{percentage}}%",
    contextSummary: "{{percentage}}% 上下文已使用",
    contextUnknown: "上下文用量未知",
    contextWaiting: "等待模型返回上下文用量",
    copied: "已复制",
    copyCode: "复制代码",
    copyOutput: "复制命令输出",
    copiedCode: "代码已复制",
    copiedOutput: "命令输出已复制",
    fileTooLarge: "{{name}} 超过大小限制",
    folderCollapse: "收起文件夹 {{name}}",
    folderExpand: "展开文件夹 {{name}}",
    invalidFileType: "{{name}} 的文件类型不受支持",
    nextBranch: "下一个回复分支",
    previousBranch: "上一个回复分支",
    removeAttachment: "移除 {{name}}",
    scrollToBottom: "回到底部",
    status: {
      completed: "已完成",
      error: "失败",
      inProgress: "进行中",
      pending: "等待中",
      denied: "已拒绝",
      responded: "已响应",
      running: "运行中",
      waitingApproval: "等待批准",
    },
    streamingOutput: "正在接收命令输出",
    tooManyImages: "最多添加 {{count}} 张图片",
    totalAttachmentSizeExceeded: "附件总大小超过限制",
    totalImageSizeExceeded: "图片总大小超过限制",
    toolError: "错误",
    toolInput: "参数",
    toolResult: "结果",
  },
  errors: {
    addProject: "添加项目失败",
    deleteProject: "无法删除项目",
    renameProject: "无法重命名项目",
    reorderProject: "保存项目排序失败",
    reorderProjectChanged: "项目列表已变化，请重试排序",
    stopTerminal: "停止终端失败",
  },
  inspector: {
    attachmentDetail: "附件",
    changeIndicator: "{{path}}，新增 {{additions}} 行，删除 {{removals}} 行",
    changes: "变更",
    closeFile: "关闭文件",
    commit: "提交",
    context: "上下文",
    contextSources: "上下文来源",
    emptyContext: "暂无上下文",
    emptyFolder: "空文件夹",
    fileTree: "项目文件",
    file: "文件",
    gitChanges: "未提交变更",
    gitChangesCount: "{{count}} 个变更",
    gitChangesRetrying: "Git 变更刷新失败，正在自动重试",
    gitLoading: "正在读取 Git 变更...",
    goal: "目标",
    goalClear: "清除目标",
    goalDuration: "{{minutes}} 分 {{seconds}} 秒",
    goalMutationFailed: "无法更新目标，请重试",
    goalPause: "暂停目标",
    goalResume: "恢复目标",
    goalSeconds: "{{count}} 秒",
    goalStatus: {
      active: "进行中",
      blocked: "受阻",
      budget_limited: "预算已用尽",
      complete: "已完成",
      paused: "已暂停",
      usage_limited: "用量受限",
    },
    goalTokenBudget: "{{used}} / {{budget}} tokens",
    goalTokens: "已使用 {{used}} tokens",
    history: "历史",
    loading: "正在读取...",
    mcpEnabled: "当前任务的 MCP 服务",
    mcpReload: "重新加载 MCP",
    mcpReloading: "正在重新加载 MCP",
    mcpStatus: {
      authenticationRequired: "需要认证",
      cancelled: "已取消",
      connected: "已连接",
      disabled: "已禁用",
      failed: "启动失败",
      notStarted: "未启动",
      starting: "正在启动",
      unknown: "状态未知",
    },
    project: "项目",
    mcpToolCount: "{{count}} 个工具",
    openChanges: "查看 {{count}} 个未提交变更",
    openFailed: "无法使用所选应用打开目标",
    plan: "计划",
    planStatus: {
      completed: "已完成",
      in_progress: "进行中",
      pending: "待处理",
    },
    projectFilesEmpty: "当前项目没有可显示的文件",
    projectFilesError: "无法读取项目文件",
    projectFilesLoading: "正在读取项目文件...",
    readFolder: "正在读取文件夹 {{name}}",
    readFolderError: "无法读取文件夹 {{name}}",
    reading: "正在读取",
    refreshFolder: "重新读取文件夹 {{name}}",
    refreshGit: "手动刷新 Git 变更",
    refreshProject: "刷新项目 {{name}}",
    refreshProjectFiles: "重新读取项目文件",
    commitChanges: "提交 {{count}} 个未提交变更",
    source: "来源",
    showMore: "显示更多",
    sourceRole: {
      admin: "管理员",
      repo: "项目",
      system: "系统",
      user: "用户",
    },
    subagents: "子代理",
    subagentCount: "{{count}} 个子代理",
    subagentOutput: "查看子代理 {{nickname}} 的输出",
    terminalRunning: "终端运行中",
    terminalStop: "停止终端 {{command}}",
    terminalStopping: "正在停止 {{command}}",
    terminalStopRetry: "停止终端失败，请重试",
    terminals: "运行中的终端",
    title: "运行环境",
  },
  gitHistory: {
    branch: "当前分支：{{branch}}",
    branchLoading: "当前分支：读取中...",
    commits: "提交记录",
    copyAuthor: "复制提交人",
    copyHash: "复制完整提交哈希值",
    copyMenuLabel: "复制提交内容",
    copyMessage: "复制提交信息",
    empty: "当前仓库暂无提交记录",
    diffLoadError: "无法读取此文件的提交 Diff",
    diffLoading: "正在读取文件 Diff...",
    diffTruncated: "Diff 过长，仅展示前 512 KiB",
    filesEmpty: "此提交没有可审核的文件",
    filesLoadError: "无法读取提交文件",
    filesLoading: "正在读取提交文件...",
    end: "已加载全部提交",
    loadError: "无法读取 Git 历史",
    loadMore: "加载更多",
    loadMoreFiles: "加载更多文件",
    loading: "正在读取 Git 历史...",
    loadingMore: "正在加载...",
    loadingMoreFiles: "正在加载文件...",
    repositories: "子仓库",
    retry: "重试",
  },
  notification: {
    completed: "Task 已完成",
    failed: "Task 运行失败",
    failedWithMessage: "Task 运行失败：{{message}}",
    interrupted: "Task 已中断，无法继续",
    waitingApproval: "Task 等待审批",
    waitingInput: "Task 等待输入",
  },
  taskTitle: {
    replying: "正在回复",
  },
  reorder: {
    cancelled: "已取消项目排序",
    moved: "{{name}} 已移动到第 {{position}} 位",
    projectFallback: "项目",
    started: "已进入项目排序，拖动后释放以保存位置",
    title: "拖动排序；也可按 Alt + 上下方向键移动",
  },
  subagent: {
    count: "{{count}} 个子代理",
    defaultName: "子代理 {{index}}",
    operationCompleted: "子代理操作已完成",
    operationFailed: "{{count}} 个子代理执行失败",
    operationRunning: "{{count}} 个子代理正在执行",
    operationSucceeded: "{{count}} 个子代理已完成",
    operationTitles: {
      close: "关闭子代理",
      followupTask: "向子代理追加任务",
      interrupt: "中断子代理",
      list: "列出子代理",
      resume: "恢复子代理",
      sendInput: "向子代理发送消息",
      sendMessage: "向子代理发送消息",
      spawn: "启动子代理",
      wait: "等待子代理",
    },
    orchestrating: "正在协调子代理",
  },
  timeline: {
    approvalReview: {
      action: {
        command: "命令",
        file_change: "文件变更",
        mcp_tool_call: "MCP 工具",
        network_access: "网络访问",
        permissions: "权限请求",
        terminal_input: "终端输入",
      },
      actionDetail: "{{action}}：{{detail}}",
      authorization: "用户授权：{{level}}",
      authorizationLevel: {
        high: "高",
        low: "低",
        medium: "中",
        unknown: "未知",
      },
      rationale: "审批理由：{{rationale}}",
      reviewing: "正在自动审批",
      risk: "风险：{{level}}",
      riskLevel: {
        critical: "严重",
        high: "高",
        low: "低",
        medium: "中",
      },
      status: {
        aborted: "已中止",
        approved: "已批准",
        denied: "已拒绝",
        in_progress: "审批中",
        timed_out: "已超时",
      },
      title: "自动审批：{{status}}",
    },
    aiRunning: "AI 回复正在运行",
    aiRunningOperation: "AI 回复正在运行：{{operation}}",
    attachments: "消息附件",
    changedFiles: "本次修改了 {{count}} 个文件",
    collapseFiles: "收起文件列表",
    collapseProcess: "收起执行过程",
    closeImagePreview: "关闭图片预览",
    commandPending: "正在执行命令",
    conversation: "会话内容",
    copied: "已复制",
    copyMessage: "复制消息",
    downloadAttachment: "下载附件 {{name}}",
    openAttachment: "打开附件 {{name}}",
    emptyAfter: "中做些什么？",
    emptyBefore: "我们应该在",
    expandProcess: "展开执行过程",
    fileChange: "{{operation}} {{name}}，新增 {{additions}} 行，删除 {{removals}} 行，打开 Diff",
    fileOperation: {
      create: "已创建",
      delete: "已删除",
      update: "已编辑",
    },
    fork: "复制任务",
    forkFailed: "复制任务失败，请重试",
    forking: "正在复制任务",
    loading: "正在加载任务历史",
    loadError: "无法加载任务历史",
    loadingOlder: "正在加载更早记录...",
    loadOlder: "加载更早记录",
    loadOlderError: "无法加载更早记录，请重试",
    jumpToMessage: "跳转到用户消息 {{index}}",
    moreFiles: "再显示 {{count}} 个文件",
    noHistory: "此任务暂无历史",
    output: "输出",
    outputOmitted: "命令输出已省略 {{bytes}} 个 UTF-8 字节和 {{lines}} 行，显示开头和结尾内容。",
    outputRetained: "较早的命令输出已按内存预算清理",
    operationGroup: {
      collapse: "收起执行详情：{{summary}}",
      commandsOnly: "命令执行完成：{{count}} 条",
      expand: "展开执行详情：{{summary}}",
      failed: "；其中 {{count}} 项失败",
      filesAndCommands: "操作完成：变更 {{fileCount}} 个文件，执行 {{commandCount}} 条命令",
      filesAndTools: "操作完成：变更 {{fileCount}} 个文件，调用 {{toolCount}} 个工具",
      filesOnly: "文件变更完成：{{count}} 个",
      summary: "操作完成：调用 {{toolCount}} 个工具，执行 {{commandCount}} 条命令",
      summaryWithFiles:
        "操作完成：变更 {{fileCount}} 个文件，调用 {{toolCount}} 个工具，执行 {{commandCount}} 条命令",
      toolsOnly: "工具调用完成：{{count}} 个",
    },
    buildPlan: "构建",
    plan: "计划",
    planGenerating: "正在生成计划",
    planReady: "计划已生成，可开始构建",
    processing: "已处理",
    quickNavigation: "用户消息快捷导航",
    reconnecting: "实时连接恢复中",
    review: "审核",
    reviewMode: "审查模式",
    reviewPrompt: {
      baseBranch: "请检查我相对于 {{branch}} 的更改",
      commit: "请检查提交 {{sha}}",
      commitWithTitle: "请检查提交 {{sha}}：{{title}}",
      custom: "请按以下要求检查代码：{{instructions}}",
      uncommitted: "请检查我未提交的更改",
    },
    running: "正在运行",
    runningOperation: "正在运行 {{operation}}",
    selectProject: "选择新聊天项目",
    showImage: "查看图片 {{name}}",
    skillsUsed: "使用的 Skills",
    editedFiles: "已编辑 {{count}} 个文件",
    editingFiles: "正在编辑 {{count}} 个文件",
    liveDiff: "实时文件变更",
    liveFileChange: "正在修改 {{name}}，新增 {{additions}} 行，删除 {{removals}} 行",
    notice: {
      guardian_warning: "安全警告",
      hook_status: "Hook 状态",
      model_verification: "模型验证",
      modelVerification: "正在验证模型可用性",
      runtime_warning: "运行时警告",
      strict_review_required: "需要严格审核",
      strictReviewRequired: "安全审核已升级，当前操作将在严格审核完成后继续",
    },
    runtimeStatus: {
      duration: "耗时 {{duration}} 毫秒",
      fasterModel: "可使用更快的模型：{{model}}",
      hook: "正在运行 Hook {{eventName}}",
      modelRerouted: "模型已从 {{fromModel}} 切换到 {{toModel}}",
      safetyBuffering: "正在使用安全缓冲模型 {{model}}",
    },
  },
} as const;
