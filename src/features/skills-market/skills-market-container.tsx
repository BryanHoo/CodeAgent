  ...
  return (
    <li className="skills-installed-row">
      <button
        ...
        type="button"
      >
        <span className="skills-market-glyph" aria-hidden="true"><Blocks /></span>
...
function SkillCard({ skill, onOpen }: Readonly<{
  onOpen: (skill: ClawhubSkillSummary) => void;
  skill: ClawhubSkillSummary;
}>) {
  return (
    <button className="skills-market-card" onClick={() => onOpen(skill)} type="button">
      ...
      <span className="skills-market-card__footer">
        <span><Download aria-hidden="true" />{skill.downloads.toLocaleString()}</span>
        <span><Star aria-hidden="true" />{skill.stars.toLocaleString()}</span>
      </span>
    </button>
  );
}
...
export function SkillsMarketContainer({ projectId, rootPath }: Readonly<{
  ...
  const install = useMutation({
    ...
    mutationFn: ({ projectId: targetProjectId, rootPath: targetRootPath, scope, skill }: {
      projectId?: string;
      rootPath?: string;
      scope: SkillInstallScope;
      skill: ClawhubSkillSummary;
    }) => client.installClawhubSkill(
      skill.owner,
      skill.slug,
      scope,
      targetProjectId,
  ...
  const groups = useMemo(() => {
    ...
    for (const skill of installed.data?.data ?? []) {
      if (skill.scope === "system" || skill.scope === "admin") {
        ...
        global.push(skill);
      } else if (skill.scope === "repo" && skill.projectId !== undefined) {
        (projectSkills.get(skill.projectId) ?? unassignedProject).push(skill);
      } else {
        unassignedProject.push(skill);
      }
    }
    return [
      ...
      {tab === "installed" ? (
        <div className="skills-market-pane">
          <div className="skills-market-toolbar">
            ...
            <Button aria-label={t("skillsMarket.refresh")} disabled={installed.isFetching} onClick={() => void installed.refetch()} size="icon-sm" title={t("skillsMarket.refresh")} type="button" variant="ghost">
              <RefreshCw className={installed.isFetching ? "animate-spin" : ""} aria-hidden="true" />
            </Button>
          </div>
          {installed.isPending ? <div className="skills-market-state" role="status">{t("skillsMarket.loading")}</div> : installed.error !== null ? <div className="skills-market-state" role="alert">{t("skillsMarket.loadError")}</div> : groups.length === 0 ? <div className="skills-market-state">{t("skillsMarket.emptyInstalled")}</div> : (
            <div className="skills-installed-groups">
              {groups.map((group) => (
                <section className="skills-installed-group" key={group.id}>
                  <header><h3>{group.title}</h3><span>{group.skills.length}</span></header>
                  <ul>{group.skills.map((skill) => <InstalledRow key={skill.id} open={(item) => openSkill.mutate(item.path)} pending={toggle.isPending && toggle.variables?.id === skill.id} skill={skill} toggle={(item) => toggle.mutate(item)} />)}</ul>
                </section>
              ))}
            </div>
          )}
        ...
        <div className="skills-market-pane">
          <div className="skills-market-toolbar">
            ...
            <Select onValueChange={setSort} value={sort}><SelectTrigger aria-label={t("skillsMarket.sort")}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="recommended">{t("skillsMarket.sortRecommended")}</SelectItem><SelectItem value="downloads">{t("skillsMarket.sortDownloads")}</SelectItem><SelectItem value="updated">{t("skillsMarket.sortUpdated")}</SelectItem></SelectContent></Select>
          </div>
