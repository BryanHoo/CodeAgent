import { ChevronDown, ChevronLeft, ChevronRight, ChevronUp, MessageCircleQuestion } from "lucide-react";
...
function QuestionDockContent({ entries, session }: Readonly<{
  ...
  return (
    <section aria-label={t("asyncQuestions.pending")} className="shrink-0 min-w-0 bg-content px-5 pb-2">
      <div className="flex min-w-0 items-center gap-2 border-t border-separator pt-2 pb-1">
        ...
        {pending.length < 2 ? null : (
          <>
            <DockButton label={t("asyncQuestions.previous")} disabled={selectedIndex === 0}
            ...
            <DockButton label={t("asyncQuestions.next")} disabled={selectedIndex === pending.length - 1}
              onClick={() => setSelectedKey(pending[selectedIndex + 1]!.key)}>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button type="button" variant="ghost" size="icon-sm" aria-controls={contentId}
              aria-expanded={!collapsed} aria-label={t(collapsed ? "asyncQuestions.expand" : "asyncQuestions.collapse")}
              onClick={() => setCollapsed((value) => !value)}>
              {collapsed ? <ChevronUp className="size-3.5" /> : <ChevronDown className="size-3.5" />}
            </Button>
          </TooltipTrigger>
          <TooltipContent>{t(collapsed ? "asyncQuestions.expand" : "asyncQuestions.collapse")}</TooltipContent>
        </Tooltip>
      </div>
...
function DockButton({ label, disabled, onClick, children }: Readonly<{
  label: string;
  disabled: boolean;
  onClick: () => void;
