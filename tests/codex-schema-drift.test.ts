...
describe("Codex Schema 漂移门禁", () => {
  it("显式更新并验证包含实验 API 的规范化基线", () => {
    ...
    expect(updateResult.stdout).toContain("Codex Schema baseline updated: 0.146.0");
    expect(verifyResult.status).toBe(0);
    expect(verifyResult.stdout).toContain("Codex Schema baseline verified: 0.146.0");
  ...
  it("拒绝新增、删除和内容变化的生成结果", () => {
    ...
    expect(result.stderr).toContain("Codex Schema drift detected for 0.146.0");
  ...
  it("忽略 JSON Schema 对象键顺序变化", () => {
    ...
    expect(result.stdout).toContain("Codex Schema baseline verified: 0.146.0");
  });
  ...
  it("缺少当前 Codex 版本基线时拒绝校验", () => {
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain("Codex Schema baseline is missing for 0.146.0");
