...
void test("Legacy preview metadata should preserve the prerelease version", () => {
  const manifest = legacyUpdateManifest("0.1.11-beta.1", "signature", "BryanHoo/CodeAgent");
  assert.equal(manifest.version, "0.1.11-beta.1");
  assert.match(manifest.platforms["darwin-x86_64"].url, /v0\.1\.11-beta\.1\/CodeAgent_0\.1\.11-beta\.1_x64_legacy/);
});

void test("Legacy updates should reference only the signed Intel Legacy asset", () => {
  const manifest = legacyUpdateManifest("0.1.10", "signature", "BryanHoo/CodeAgent");
  assert.deepEqual(manifest.platforms, {
    "darwin-x86_64": {
      signature: "signature",
      url: "https://github.com/BryanHoo/CodeAgent/releases/download/v0.1.10/CodeAgent_0.1.10_x64_legacy.app.tar.gz",
    },
  });
  assert.throws(() => legacyUpdateManifest("0.1.10", "", "BryanHoo/CodeAgent"));
});
