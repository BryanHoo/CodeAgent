...
fn codex_version_should_require_the_verified_protocol_version() {
    assert_eq!(
        parse_codex_version("codex-cli 0.154.0\n"),
    ...
    assert_eq!(parse_codex_version("codex-cli 0.154.0-beta.1\n"), None);
    assert_eq!(parse_codex_version("codex-cli 0.154.0+build\n"), None);
    ...
    assert_eq!(parse_codex_version("codex-cli 0.154.0 unexpected\n"), None);
}

#[tokio::test]
#[ignore = "downloads the private codex-cli 0.154.0 binary"]
