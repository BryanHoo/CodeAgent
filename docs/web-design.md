...
Codex App Server 暴露的是 `Thread -> Turn -> Item` 模型。Web 必须保留这些语义，不能把所有内容压平成 `user/assistant` 消息数组。
...
- 在高频 Delta、长会话和多 Thread 场景下保持输入和滚动流畅。
...
flowchart LR
    ...
    Query["TanStack Query"] --> Runtime["Thread Runtime Store"]
    Client --> Query
    Client --> Buffer["Event Order + Delta Buffer"]
    Buffer --> Runtime

    Runtime --> VM["Agent ViewModel"]
    VM --> Timeline["ThreadTimeline"]
...
| 路由         | `@tanstack/react-router`      | 管理 Workspace、Thread、Settings 深链接  |
| HTTP 状态    | `@tanstack/react-query`       | 管理 Snapshot、分页、Mutation 和缓存失效 |
| 实时状态     | `zustand/vanilla`             | 创建 Thread 级归一化 Store               |
...
| ---------------- | ----------------------------------- | ----------------------------- |
| `conversation`   | 会话滚动、底部跟随、回到底部按钮    | 包装进 `ThreadTimeline`       |
...
| 路由                          | 页面职责                                      |
| ----------------------------- | --------------------------------------------- |
| `/`                           | 检查 Runtime、认证和最近 Workspace，并重定向  |
| `/login`                      | 展示 ChatGPT 登录、Device Code 或错误恢复     |
| `/workspaces`                 | 选择或添加允许的 Workspace                    |
| `/w/:workspaceId`             | 显示 Workspace 空会话状态并允许创建 Thread    |
| `/w/:workspaceId/t/:threadId` | 主工作台，显示 Thread 和 Composer             |
| `/settings`                   | 模型、Reasoning、Sandbox、Approval 和外观设置 |

Workspace 和 Thread ID 必须来自 Server，URL 只表示导航意图，不能代替权限校验。
...
```text
+----------------+--------------------------------+------------------+
| Workspace /    | Thread Timeline                | Inspector        |
| Thread Sidebar |                                | Diff / Details   |
|                |                                |                  |
|                |                                |                  |
|                | Composer                       |                  |
+----------------+--------------------------------+------------------+
...
- Sidebar 默认宽度约 `280px`，支持折叠。
- Timeline 占据剩余空间，宽度使用合理上限保证 Markdown 可读性。
- Inspector 默认关闭，仅在 Diff、File Change 或详细信息打开时出现。
...
| 状态                   | 所有者                | 示例                                      |
| ---------------------- | --------------------- | ----------------------------------------- |
| HTTP Server State      | TanStack Query        | Workspaces、Models、Thread Page、Snapshot |
| Realtime Runtime State | Zustand Vanilla Store | 活动 Turn、Item Delta、Approval、Usage    |
| Route State            | TanStack Router       | 当前 Workspace、Thread、Settings 页面     |
| Local UI State         | 最近组件              | 展开状态、Popover、临时选择、Hover        |
| Draft State            | Composer Feature      | Thread 草稿、附件和输入模式               |
...
models
workspaces
threads.list(workspaceId, filters, cursor)
...
- Thread 列表使用 Cursor Pagination。
- Snapshot 获取成功后 Hydrate 对应 Thread Runtime Store。
- Mutation 成功后只失效受影响的低频 Query。
- 不为每个 Delta 调用 `queryClient.setQueryData`。
- Thread 切换时可以预取目标 Snapshot，但不能产生请求瀑布。

### 12.3 Thread Runtime Store

使用 `zustand/vanilla` 的 `createStore` 创建独立 Thread Store，通过 Context 注入 Store 引用。建议状态形状：
...
- 未选中且无活动 Turn 的 Thread Store 可以按 LRU 回收。
...
flowchart LR
    ...
    FlushRelated --> Reduce
    Reduce --> Store["Thread Runtime Store"]
```
...
3. 请求当前 Thread Snapshot。
4. 原子替换 Thread Runtime Store。
...
- 获取 Snapshot 校正 Thread、Turn、Item 和 Pending Request。
...
- 首次打开 Thread 时定位到最新内容。
- 用户位于底部时，新 Item 和流式内容自动跟随。
- 用户离开底部后，不再抢占滚动位置。
- 新内容到达时显示“回到底部”按钮或未读提示。
- 加载更早历史时保持当前视觉锚点。
- 打开 Diff Inspector 不改变 Timeline 的滚动位置。
- 切换 Thread 时恢复各自最近滚动位置，恢复失败则定位到底部。

### 15.3 虚拟化升级条件

AI Elements `Conversation` 会保留已渲染 Item 的 DOM。只有真实性能数据满足以下任一条件时，才将 `ThreadTimeline` 内部替换为 `react-virtuoso`：

- 常见 Thread 达到约 `300-500` 个已加载 Item。
- Timeline DOM 节点持续超过约 `2,000-3,000`。
- 打开长 Thread 的前端渲染耗时超过 `200ms`。
- 流式输出或滚动期间频繁出现超过 `50ms` 的 Long Task。
- 滚动帧预算持续超过 `16ms`，产生用户可见卡顿。
- Thread 切换后内存无法回落到稳定范围。
...
- Workspace 文件引用转换为项目内部动作，交给 Server 重新执行权限和路径校验。
...
- 草稿按 `workspaceId + threadId` 隔离。
- 小型纯文本草稿可以存入 `localStorage`。
- 附件只保存可恢复的元数据，不持久化短生命周期 Blob URL。
- Thread 删除、归档或 Workspace 移除时清理对应草稿。
...
提交 Approval 时必须携带 Request ID，并由 Server 再次校验 Runtime、Thread、Turn、Item、用户和状态。按钮点击后立即进入提交态，防止重复操作。
...
| Reconnecting           | 保留当前 Thread，显示连接状态并暂停危险 Mutation |
...
单个未知或损坏 Item 不得导致整个 Thread 无法使用。
...
- Workspace 文件操作每次由 Server 校验绝对路径、`realpath` 和允许根目录。
...
- 不在父级 Thread 组件读取完整 `itemsById` 对象。
...
- 未选中且无活动 Turn 的 Thread Runtime Store 允许回收。
- Command Output、Event Buffer 和诊断数据必须有界。
- Blob URL 在附件删除、提交成功或组件卸载时释放。
- Query Cache 为 Thread Page 和 Snapshot 设置合理 GC 时间。
...
- 实现 Client HTTP 基础调用。
- 完成登录、Workspace、App Shell 和 Thread Sidebar。

完成标准：可以从启动页进入一个 Workspace，并显示 Thread 列表和明确状态。

### 阶段二：Conversation Read Path

- 选择性引入 AI Elements Conversation、Message 和 Reasoning。
- 实现 Thread Snapshot Query。
- 实现 Thread Runtime Store 和 Snapshot Hydration。
- 实现 AgentItemRenderer 和基础 ViewModel。
- 实现分页历史与滚动恢复。

完成标准：可以稳定读取 Thread，并正确显示结构化历史。
...
原因：真实长历史规模尚未建立，先用简单完整的滚动体验。通过 `ThreadTimeline` 隔离实现，达到明确性能阈值后再替换。
...
原因：本项目的数据模型是 Thread、Turn、Item、Approval 和可恢复 Agent Event。引入 AI SDK Runtime 会形成重复状态和有损转换。
...
- 流式 Item 独立订阅，没有导致整个 Thread 重渲染。
