...
| Packaging          | clean VM                          | 三平台未签名安装、启动、安全提示和更新              |
...
- [x] macOS 仅发布 14+ Apple Silicon artifact，并在 release runner 验证最低系统版本、安装与启动。
- [x] 三平台 Desktop 以 Preview / Unsigned 构建，并在最低支持系统验证安装与启动。
...
| 发布矩阵显著增加 CI 时间                 | PR 使用分层门禁，三平台 installer 测试只在 release/daily workflow 执行              |
...
- [ ] 性能预算、bundle budget、native package 与三平台 Preview / Unsigned 门禁进入 CI。
