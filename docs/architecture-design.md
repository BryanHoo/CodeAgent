...
| `startTurn(goalMode)` | `thread/settings/update` + `thread/goal/set` |
...
`goalMode: true` 不进入普通 `turn/start`。Provider 先调用 `thread/settings/update` 应用同一组选项，再以正文设置 active Goal，并等待 `thread/goal/set` 自动产生的 `turn.started`。Goal 更新通知不向上扩展 Protocol；自动 Turn 产生的 Message、Tool 和终态继续复用既有事件映射。
...
App Server 返回 `code = -32001` 且 `data.retry = true`、明确表示请求未入队时，Adapter 使用带正负 `20%` jitter 的有上限指数退避，默认最多重试 `4` 次、累计退避不超过 `5s`、单次不超过 `2s`，并沿用首次请求总超时；连接关闭或超时会取消待执行重试，其他错误直接透传。
