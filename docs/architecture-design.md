...
- `roots[0]` 是 primary folder，供 Task cwd、AGENTS、配置和 Skill 使用。
...
Web 只保存 `{ projectId, path }` 形式的当前根选择，并始终根据最新 Project roots 派生有效值。缺少选择、项目变化或根被 Codex 移除时回退首根。根切换不会改变 Task Runtime，只同步更新中栏路径/分支和右栏文件、变更、历史、预览等根级视图。
