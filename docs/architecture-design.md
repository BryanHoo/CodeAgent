...
命令启动后，用户直接在浏览器中完成登录、Workspace 选择、Thread 管理、任务执行、审批、Diff 查看和中断操作。
...
- 支持 Thread、Turn、Item、流式输出、审批、Diff、Plan 和 Usage。
- 默认采用本地优先、安全收敛的运行方式。
- 在长会话、高频流式输出和多 Thread 场景下保持稳定性能。
...
flowchart LR
    ...
    AppServer --> Workspace["Local Workspace"]
...
默认每个 `CODEX_HOME` 只运行一个长驻 Codex App Server，并由该进程承载多个 Thread。不要为每个 Turn 创建新的 Codex 进程。
...
export interface AgentProvider {
  ...
  listThreads(input: ListThreadsInput): Promise<Page<AgentThread>>;
  readThread(input: ReadThreadInput): Promise<AgentThread>;
...
Thread、Turn、Item 的层级与 Codex 原始模型一致：
...
Codex 已经持久化 Thread 和 Session 历史。CodeAgentWindow 不逐 Token 重复存储完整历史，只维护统一 Read Model 和断线恢复所需数据。
...
- 使用 TanStack Virtual 渲染 Thread 和 Item 长列表。
- 使用 Normalized Store，以 ID 关联实体。
- 每个 Item 独立订阅，避免整个 Thread 重渲染。
- Markdown 只重新解析当前流式消息。
- 代码高亮延迟到代码块稳定后执行。
- Diff 使用 `turn/diff/updated` 的完整快照，不自行拼接历史 Patch。
- CodeMirror 仅在用户打开文件或 Diff 编辑功能时动态加载。
...
每次 HTTP 和 WebSocket 操作都必须检查 Workspace 权限。
...
其中 `thread/shellCommand` 不继承 Thread Sandbox，而是以 Full Access 执行，不能作为普通聊天功能开放。
...
5. 使用有上限的指数退避重启。
6. 重新执行 initialize。
7. 通过 thread/resume 恢复活动 Thread。
...
| 长列表        | TanStack Virtual    | 控制长历史 DOM 数量                           |
| 编辑器        | CodeMirror 6        | 相对轻量，支持按需加载                        |
...
| 长历史数据量大            | 打开 Thread 卡顿     | Snapshot、分页、Read Model、虚拟列表 |
...
不采用。它会增加进程启动开销，也无法自然支持完整 Thread Runtime、双向审批和长期订阅。
