...
这是顺序编排，不是原子事务或持久化提交日志。创建与启动仍分别使用原键及各自保留窗口；创建结果不确定、通知和最终响应均丢失、应用重启后的整体恢复仍未解决。Review、steer 与排队编排仍待迁移。
...
| 2 | Review/steer/排队编排及整体恢复 | 普通创建后启动已原生编排并保留部分成功摘要；继续完善跨调用恢复、失败重试交互和其他执行幂等 |
