...
前端继续承担草稿、表单提示和提交展示；本批未改动前端现有输入修剪行为。每题单项是 CodeAgent 当前表单契约，上游 `RequestUserInputAnswer.answers` 本身允许字符串数组；不新增选项标签白名单。异步 `agentMessage.questions` 的历史文本对账仍未迁移。
