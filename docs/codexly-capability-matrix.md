...
| 队列追加与清理恢复 | `steerTurn`、`startQueuedSubmission` → `turn/steer`、`thread/queue/delete` | Rust 保留已接受结果；运行中换键或换回合只补清理，转为空闲后启动入口也只清理已接受项。下一项请求固定首项身份，清理前核对内容，`cleanupOnly` 不生成新回合消息。保留 15 分钟，不保证原子消费或跨重启恢复 | 已迁入 Rust |
