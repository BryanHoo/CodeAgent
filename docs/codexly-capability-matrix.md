...
| 运行中队列追加与清理恢复 | `steerTurn` → `turn/steer`、`thread/queue/delete` | Rust 顺序执行并保留队列项的已接受结果；15 分钟内同内容换键或换回合只补清理，内容变化拒绝，`cleanupOnly` 不生成新回合消息。不保证原子消费、空闲启动路径或跨重启恢复 | 已迁入 Rust |
