...
更新按已安装档位隔离，不会自动从 Legacy 升到 Modern。更新提示复用 GitHub 发布响应，只有当前渠道的清单上传后才提供更新。系统升级后可手动替换同名 `CodeAgent.app`；应用 identifier 不变，因此继续使用原有本地数据。实际发布和更新签名需要 CI 中已有密钥，本地验证不代替发布签名验收。
...
xcrun vtool -show-build src-tauri/target/legacy/x86_64-apple-darwin/debug/codeagent
```
