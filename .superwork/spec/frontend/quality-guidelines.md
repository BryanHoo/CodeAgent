...
- `pnpm test:performance` 必须在真实 Chromium 中同时覆盖固定 10,000 Item 长历史和单 Turn 10,000 Tool Item 病理场景，验证 Turn/Item 虚拟挂载、DOM、绘制、交互延迟、long task、CPU、Heap 与 GC，并以固定实时样本采集 `provider_received`、`runtime_published`、`transport_received`、`store_committed`、`painted`，分别断言端到端 `p50/p95/p99`；病理场景必须单独限制挂载 Item 数和稳定 DOM 节点数，不得用 `renderToStaticMarkup` 结果代替浏览器性能门禁。规模与阈值只维护在 `tests/performance-budgets.json`。
