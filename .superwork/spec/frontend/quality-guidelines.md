...
- Inspector、Diff、源码预览和全局设置必须保留独立动态加载入口，关闭状态不得把对应实现静态引入首屏。Production build 必须生成 Vite manifest，`pnpm run bundle:check` 沿静态 import 图限制首屏 JavaScript 为 `240 KiB gzip`，并限制任一异步入口及其首屏外静态依赖为 `200 KiB gzip`；不得提高 `chunkSizeWarningLimit` 或预算来掩盖依赖回退。
- 测试断言用户可观察行为，不复制实现细节。
