...
- `pnpm test:performance` 必须在真实 Chromium 中以固定 10,000 Item 历史验证归一化、Turn 虚拟挂载、DOM、绘制、交互延迟、long task、CPU、Heap 与 GC，并以固定实时样本采集 `provider_received`、`runtime_published`、`transport_received`、`store_committed`、`painted`，分别断言端到端 `p50/p95/p99`；另以单个包含 10,000 项已完成普通操作的 Turn 验证默认折叠只挂载最近 20 项、语义摘要、DOM、Hydration 与 GC 后保留 Heap。不得用 `renderToStaticMarkup` 结果代替浏览器性能门禁。规模与阈值只维护在 `tests/performance-budgets.json`。
