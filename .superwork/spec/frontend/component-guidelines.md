...
- Timeline 展示 Task Snapshot 中的 Agent 文件操作；Inspector 的“变更”页签顶部保留当前 Project 的真实 Git 未提交变更总览。总览左侧在“未提交变更”下方依次同行展示变更总数、增加行数和删除行数，右侧并排展示使用相同中性背景的“审核”与“提交”按钮。“审核”使用统一文件审核 Dialog 浏览全部未暂存与已暂存 Diff；“提交”当前只展示入口，不绑定 Git Mutation。总览下方使用本地 AI Elements `FileTree` 展示受限 Project 文件树；目录默认折叠，用户展开时才加载其直接子项，折叠父目录时清理后代展开状态，文件点击复用受控源文件弹窗；根目录和各级子目录的加载、空与错误状态必须明确。当前 Task 运行时 Inspector 定时刷新 Git 状态，停止运行后补做最终刷新；连续失败时停止自动轮询并提供手动刷新。完整 Diff Viewer 使用 `@pierre/diffs/react` 并仅在打开弹窗后动态加载，不保留旧 Git 分组列表或演示数据。
