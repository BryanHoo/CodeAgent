...
- `Projects` 标题使用高于分组元数据的字号并固定在项目树滚动区域之外；Project 行之间不增加分组间隔。每个 Project 的 Task 首屏只请求并展示最近 5 项；“显示更多”每次只按该 Project 的 Cursor 请求一个下一页，加载失败保留已有 Task 并允许重试，全部加载后可通过同样的整行控件收起。搜索只匹配当前已加载 Task，不得为了搜索自动读取完整历史。
