...
- Timeline 与 Inspector 的文件变更必须消费同一 Task Snapshot；新增或删除文件的行数统计同时支持 Unified Diff 和 Provider 返回的完整文件内容；文件行作为 Diff 弹窗入口，完整 Viewer 使用 `@pierre/diffs/react` 并仅在打开弹窗后动态加载，不能在消息内展开原始补丁或保留演示变更数据。
