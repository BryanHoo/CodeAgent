...
- Composer 文本草稿必须在 Runtime 重渲染期间保留浏览器的 IME 组合缓冲，不能通过受控 `value` 回写覆盖尚未触发 `onChange` 的临时文本；程序化清空或替换草稿时仍需同步真实 `textarea`，并使用 `compositionstart` 后触发重渲染的 Playwright 用例覆盖中文首键。
