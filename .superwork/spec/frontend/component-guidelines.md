...
- 通用 Tool 必须使用 AI Elements `ToolInput` 与 `ToolOutput` 分区展示参数、JSON 结果和错误文本；`AgentItemStatus` 只在 Timeline 视图边界映射为 AI Elements Tool 执行状态，不向 Web 引入 `ToolUIPart` 或 AI SDK Runtime。已折叠 Tool 只挂载摘要，参数、输出、错误和 Terminal 在首次展开时才挂载，重新折叠后立即卸载；运行中、失败、拒绝或中断的 Tool 可以按状态默认展开。
...
- Timeline 展示 Task Snapshot 中的 Agent 文件操作；Inspector 的“变更”页签顶部保留当前 Project 的真实 Git 未提交变更总览。总览左侧在“未提交变更”下方依次同行展示变更总数、增加行数和删除行数，右侧并排展示使用相同中性背景的“审核”与“提交”按钮。“审核”使用统一文件审核 Dialog 浏览全部未暂存与已暂存 Diff；“提交”打开原生 Dialog，按 Project 相对路径合并 staged/unstaged 记录并默认全选，允许生成或编辑 message，再执行 commit 或 commit+push。提交控制器、Dialog 和 Mutation observer 只在首次打开时动态加载并注册，关闭后必须整体卸载。文件选择器默认折叠为一行摘要，点击摘要后才渲染选择明细；展开时只允许有界文件区域内部滚动，message 输入区和弹窗操作区必须保持可见。聚合子仓库模式禁用提交；commit 成功但 push 失败时 Dialog 必须保留 commit SHA 与部分成功状态。总览下方使用本地 AI Elements `FileTree` 展示受限 Project 文件树；目录默认折叠，用户展开时才加载其直接子项，折叠父目录时清理后代展开状态；点击存在 Git 变更的文件必须打开单文件 Diff 弹窗，点击未变更文件继续打开受控源码预览弹窗；根目录和各级子目录的加载、空与错误状态必须明确。Git 状态可用且存在未提交变更时，文件行最右侧展示该文件的增删行数；具体文件尚不可见时只在最深可见祖先目录最右侧展示后代汇总，并随目录展开逐层下移；已删除或被文件树规则过滤的文件没有实际节点时，汇总必须保留在最深可见祖先目录。当前 Task 运行时 Inspector 定时刷新 Git 状态，停止运行后补做最终刷新；连续失败时停止自动轮询并提供手动刷新。完整 Diff Viewer 使用 `@pierre/diffs/react` 并仅在打开弹窗后动态加载，不保留旧 Git 分组列表或演示数据。
