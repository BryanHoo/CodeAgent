...
- Project 名称行在指针移动超过点击容差后立即进入拖动排序，不设置固定长按等待；点击容差内进入对应 Project 的新聊天草稿并展开任务树，不得创建或渲染临时 Task 行；释放后保存完整 Project 顺序并播报结果；键盘必须提供 `Alt + ArrowUp/ArrowDown` 等价操作。排序过程中只保留瞬时顺序，保存失败恢复 Server 顺序并显示可见错误。
...
- Project 名称、名称右侧的可访问 `+` 图标都进入对应 Project 的“新聊天”草稿；顶部“新建任务”始终进入第一个 Project 的草稿，新增文件夹后进入该 Project 草稿，目标草稿已打开时直接复用。上述入口只能重置中栏和切换 Project，左栏不得出现临时“新聊天”Task。
