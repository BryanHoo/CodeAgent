...
- Task 级 Warning 与 Model Verification Notice 最多保留最近 20 条；Notice 和 Turn Diff 属于瞬时状态，不写回重建 Snapshot。Timeline 使用项目设计令牌适配的 Agent 组件展示 Summary、Plan、Tool Progress、运行中文件集合、Hook 与模型状态，完成过程默认折叠。
