...
- Pending Request 使用 `command_approval`、`file_change_approval`、`permissions_approval`、`user_input` 判别联合；Provider 必须在映射 User Input 前严格校验原生 `isBlocking` 为布尔值，缺失或非法值不得进入 Pending 生命周期；命令审批将受管网络目标归一化为可空的 `networkAccess`，并将可选 `additionalPermissions` 归一化为 Provider 无关权限 Profile；权限审批保留网络与文件系统请求明细，Resolution 只允许提交原请求类别的唯一子集和显式 `turn | session` 作用域；Snapshot 只返回未解决请求，实时链路使用 `pending_request.created`、`pending_request.resolved`、`pending_request.expired` 同步生命周期。
