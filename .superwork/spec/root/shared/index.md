...
- 自动触发与立即运行创建的 Task 必须通过 `scheduled-task://started` 向主窗口发送轻量 `AgentTask` 摘要；全局监听直接补入普通分页及已加载搜索缓存，并恢复任务状态订阅，不得依赖定时任务页面挂载或轮询。监听异步注册晚于组件卸载时必须立即清理。界面不显示 IANA 时区名称，保存仍保留调度时区。
