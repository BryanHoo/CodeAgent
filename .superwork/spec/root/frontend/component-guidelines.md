...
- 待回答问题固定于中栏顶部、聊天时间线上方，与中栏等宽且不进入时间线滚动容器；多组按到达顺序切换并显示待回答组数，默认展开、可折叠，内部滚动高度不超过 `min(32vh, 20rem)`。允许不回答直接关闭当前组，断线时仍可关闭；关闭仅更新会话状态、保留草稿，不提交答案或标记已发送。关闭或发送成功后移出固定区，时间线仅保留只读问题与普通用户回答；历史恢复只识别完整的本项目回答格式，不把任意后续消息推断为已回答
