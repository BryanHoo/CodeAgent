...
- 项目终端自然退出或点击结束后必须自动清理会话与 tab，不保留二次删除入口；最后一个 tab 关闭时收起整个终端区域。切换 Task 时必须收起终端面板但保留项目终端会话，禁止沿用上一 Task 的展开状态。底栏终端入口放在左侧文件路径之后，无运行终端时只显示图标、不显示数量；验证须覆盖创建响应之前退出的竞态、多 tab 选择切换及 Task 切换后的显隐状态
