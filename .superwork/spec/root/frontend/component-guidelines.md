...
- 运行中队列发送必须通过 `steer_turn` 透传 `queuedSubmissionId`，由 Rust 追加后清理；前端不得再次删除。队列项 ID 必须参与前端尝试身份，`QUEUE_CLEANUP_FAILED` 表示追加已成功，当前目标未变化时保留原键补做清理，不得自动换键重发。原生返回 `cleanupOnly: true` 时仅刷新队列，不生成当前回合的待确认消息；跨回合恢复身份由 Rust 管理，前端不建立恢复缓存。
