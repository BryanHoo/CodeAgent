...
- 提交消息的生成模型与规则归入“编码 → 个性化”，不再提供独立提交消息菜单；搜索提交相关字段应定位个性化。规则输入采用整行编辑区并展示长度，沿用自动保存和切换分类保留草稿的行为，与全局 `AGENTS.md` 的手动保存区分。

- 全局设置外壳与各分类均按需拆包，仅渲染当前分类；背景图片读取、个性化说明与记忆查询等分类专属副作用必须位于对应懒加载组件内。搜索使用轻量分类索引，不得为搜索加载所有面板；验证首次进入常规不读取背景，导航后才读取，并覆盖 Chromium/WebKit 的异步加载状态。
