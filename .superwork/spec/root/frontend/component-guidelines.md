...
- Composer 项目草稿必须仅按 `projectId` 隔离，不绑定 `taskId`；已保存版本只能由显式保存操作覆盖，编辑过程写入独立工作副本且关闭编辑不得丢失或改动已保存版本；浏览器附件必须先稳定化为宿主附件，只有发送成功后才能删除对应草稿；草稿列表标题必须使用项目名，从列表应用草稿时必须立即关闭列表，输入框非空时必须先明确确认覆盖；恢复草稿的内容和附件均被清空时必须丢弃工作副本并退出更新模式
