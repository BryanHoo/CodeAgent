...
- 任务看板必须复用 `ProjectDraftStore` 草稿与 Rust 任务活动投影；运行中、待处理只遍历有界 Activity Map，已完成由 Rust 过滤 `idle/notLoaded` 后按更新时间提供 10 条一页的跨项目 Cursor，WebView 仅维护项目过滤和 Infinite Query 页面，不提供手动拖拽改写；已完成查询失败只能降级对应列并提供重试，不得替换为全局 Runtime 不可用页面
