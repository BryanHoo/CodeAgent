...
- Project 文件树只通过固定的只读端点读取已配置根目录；端点可接收经过严格校验的 Project 相对目录，每次只返回该目录的直接子项，不提供绝对路径或文件系统透传。`.gitignore` 只作为普通文件展示，其规则不得过滤任何文件或目录；同时跳过符号链接、`.git` 与大型生成目录，并保留固定目录深度限制，不设置条目数量上限。
