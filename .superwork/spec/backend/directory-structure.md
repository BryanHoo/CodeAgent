...
- Project 宿主打开能力只返回固定白名单中的具体应用 ID、名称与类别；普通打开菜单提交 Project 相对路径，AI 文件引用允许提交相对或绝对路径。Server 必须从 Repository 读取根目录，将两类输入统一转换为 Project 内目标，逐段拒绝符号链接和越界目标，按宿主实际可执行程序或应用包过滤目录，并使用参数数组和 `shell: false` 启动。文件交给编辑器或工具，文件管理器定位文件或打开其父目录，终端固定在文件父目录启动；`system-default` 只允许文件目标，并调用宿主系统的默认关联应用。
- Project 图片预览只允许 GIF、JPEG、PNG、WebP 的有效内容签名和有界普通文件；响应固定使用受检媒体类型与 `nosniff`，路径缺失、越界、符号链接、超限或签名不匹配统一返回不可用。
