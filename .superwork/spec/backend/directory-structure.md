...
- Core 分离 `ProjectRepository` 与 `ProjectProjectionStore`：前者由 `provider-codex` 实现并保证所有 Mutation 先写 Codex，后者由 `packages/server` 的 SQLite Adapter 实现且只接受完整 Codex Project 投影。CodeAgent 公共 `Project.id` 必须原样使用 Codex `projectId`，有序 `roots[]` 必须完整投影，首项是 primary folder。
...
- Project 文件搜索必须复用文件树的 Project 根目录、生成目录、符号链接和深度边界，但继续沿根目录逐层应用任意层级的 `.gitignore`；搜索只按文件名匹配并稳定返回最多 `50` 个普通文件，每项携带稳定 `rootId`、绝对 `rootPath` 和根内相对 `path`。Composer 选择结果必须按 `rootId + path` 去重，并序列化为可见的宿主绝对 `@<path>` 文本随普通文本提交；文件夹不得进入引用入口，浏览器协议和 Provider 不再建立结构化文件 `mention`。
