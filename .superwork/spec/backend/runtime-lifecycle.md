...
- Codex `0.149.0` 的 `thread/queue/changed` 必须映射为 Provider 无关的 `queue.changed`，只携带已验证归属的 Project/Task ID；`thread/reverted`、`project/changed` 与 `thread/project/updated` 继续归入 ignored 并在初始化时 opt-out。CodeAgent 继续拥有 Project 边界，原生 Queue、Project、Revert 或 Diagnostics 结构不得直接透传到 Core、Protocol 或 Web。
...
- 新建持久化 Task 必须使用 `historyMode: "paginated"`。Task 与 Reviewer Snapshot 先调用 `thread/read { includeTurns: false }` 获取元数据，再通过 `thread/turns/list` 按时间倒序读取最多 `10` 个 Turn；Paginated Thread 使用 `itemsView: "notLoaded"` 并以 `thread/items/list` 每页最多 `100` 项水合，Legacy Thread 只允许在当前 Turn 页使用 `itemsView: "full"`。Codex Cursor 只能留在 Provider 边界，对外游标必须封装主 Thread Cursor 与 Reviewer 偏移；所有分页循环都必须拒绝重复 Cursor。
- Codex `0.149.0` Thread 必须严格校验必填的可空 `projectId`，但原生 Project ID 不得越过 Provider 边界；`agentMessage.delivery` 必须严格校验为 `null | async` 且不进入统一消息。`agentMessage.phase` 中的 `commentary` 与 `final_answer` 都必须映射为 Assistant Message、原样保留有效阶段，并通过 `message.delta` 实时交付；缺失或 `null` 阶段保持无阶段，非法阶段必须拒绝映射。原生 `reasoning` Item 仍映射为统一 Reasoning Item；Web 只允许展示 `summary`，原始 `content` 永不进入展示组件或 DOM。
...
- Codex `item/plan/delta`、Reasoning Summary 分段、MCP Progress、File Patch、Turn Diff、Hook、Safety Buffer、Model Reroute、Model Verification、Guardian Warning 与结构化 Error 必须映射为 Provider 无关事件或 Item。`autoApprovalReview/strictReviewRequired` 必须校验 Task、Turn 与非负时间戳，并映射为可本地化的统一 warning，不得把原生通知字段传到 Web。高频文本事件按 Item 追加，状态快照按 Item 或 Turn 替换；`item.completed` 和 `turn.completed` 仍是最终权威实体。`contextCompaction` 必须映射为带 `transient` 标记的统一 Activity，使开始、完成与历史 Snapshot 保持同一语义。
...
- Git worktree 必须通过 `git worktree list --porcelain -z` 有界读取；切换请求只能接受该列表中属于当前仓库且非当前项的真实绝对路径。创建 worktree 时在仓库同级为已有或新分支生成不冲突目录，并把目标目录注册为独立 Project；不得修改当前 Project 的 `rootPath` 或复用当前 Task Runtime。
...
- Server 启动不得枚举项目并预建 Runtime；Project Runtime Context 只在首次 Project API 或 WebSocket 访问时激活。已激活 Context 必须先从进程内缓存解析，只有缓存未命中时才读取 Project Repository。Project API 访问和 Provider 事件必须刷新 Context 活动时间，存在 Event WebSocket 客户端时不得按空闲淘汰；超过固定空闲期限的 Context 必须由 `unref` 周期定时器通过统一 `releaseProject` 路径释放，定时扫描不得重入。后续访问按现有单飞逻辑重新读取 Project Repository 并重建 Runtime。Project 重命名成功后同步刷新缓存中的展示信息；Project 删除成功后必须释放事件订阅和 Context 缓存，后续访问重新读取 Repository 并返回资源不存在，不能复用已删除 Runtime。
...
- Project 删除、空闲淘汰和 Server 关闭必须通过 Runtime 的显式 `releaseProject` 端口统一释放 Project Provider、原始 Provider、Task Owner、Pending Request 定时器、Task 运行状态和历史附件授权；Server 同时清理该 Project 未消费或 Turn 占用中的上传附件，且不得影响其他 Project。Server 关闭必须先停止并等待正在执行的空闲扫描，再释放剩余 Context，避免同一 Project 重叠清理。
