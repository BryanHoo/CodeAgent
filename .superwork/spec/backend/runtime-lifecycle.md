...
- Desktop Composition Root 必须有界解析一次宿主工具环境，并向 Codex、Git 与 Project 宿主打开服务显式注入同一 `PATH` 快照；Codex 只能在该快照前追加受管运行时目录，Git 应用快照后仍必须移除危险 Git 环境变量。
- Desktop Tauri `setup` 不得等待 SQLite 目录创建、连接打开、备份、迁移、完整性检查、附件目录初始化或 login shell 探测。SQLite owner thread 必须先返回有界队列句柄，数据库任务通过 Tokio `oneshot` 和有界异步超时返回结果；Repository、File 与 Git 的异步调用链不得使用 `recv`、`recv_timeout` 或其他同步结果等待占用 Tokio worker。每个数据库 Job 必须携带入队时的 deadline，worker 执行前必须跳过已过期或 receiver 已关闭的请求；caller 超时后必须中断正在执行的 SQLite 查询，并清理遗留 interrupt 标志，避免超时任务继续占用队列或在客户端超时后仍提交写入。首个数据库请求在既有超时内等待初始化结果；附件目录在首次写入时异步初始化；login shell 在后台完成后原子更新 Git、Project Open 与 Codex 共享的同一 `PATH` 快照。
...
- App Server 初始化必须启用 experimental API，并通过 `optOutNotificationMethods` 抑制 `CODEX_IGNORED_NOTIFICATION_METHODS` 中明确不消费的通知，避免无效通知进入 JSONL 解析和路由热路径；后台终端只通过 `thread/backgroundTerminals/list` 获取，并在 Provider 边界把原生 `processId` 映射为不透明 Terminal ID。停止单个终端固定调用 `thread/backgroundTerminals/terminate`，不能用 `turn/interrupt` 代替。
...
- `thread/start` 返回的新 Task 在首条用户消息前可能尚未 materialize；此时 `thread/read(includeTurns: false)` 的明确未 materialize 错误必须映射为该已知 Task 的空闲空快照，未知 Task 和其他 RPC 错误不得被吞掉。
- Task Snapshot 禁止通过 `thread/read(includeTurns: true)` 全量读取历史；固定先用 `thread/read(includeTurns: false)` 读取元数据，再用 `thread/turns/list { limit: 20, sortDirection: "desc", itemsView: "full" }` 读取最近页并按时间正序映射。统一 Snapshot 使用 `turnsNextCursor` 暴露不透明旧页游标，Runtime、N-API、HTTP、Tauri、Client 和 Web 必须通过 `AgentTurnPage` 按需加载更早 Turn，拒绝重复 Cursor 且不得覆盖已由实时事件更新的 Turn；Review 子线程只读取最新一个完整 Turn，不得恢复全量读取回退。
...
- 当前 Task 可读取的 MCP 服务只通过 `mcpServerStatus/list { threadId, detail: "toolsAndAuthOnly" }` 分页获取，并与同一 Task 的 `mcpServer/startupStatus/updated` 合并；Provider 必须先确认 Task 归属当前 Project，历史或已释放 Task 必须复用并发去重的 `thread/resume` 完成加载后再查询或重载 MCP。有效启动通知必须发布为统一 `mcp_server.status_updated` 事件，使 Web 能按服务实时区分 `starting`、`ready`、`failed` 和 `cancelled`。只向 Core、Protocol 和 Web 暴露名称、启动状态、工具数量、认证方式、展示元数据、失败原因及 Codex 原始启动错误，不得截断、脱敏或改写错误；工具定义、资源、command、args、env 等非错误 Provider 数据禁止越过 Provider 边界。不得使用无 `threadId` 的进程级结果，也不得回退到 Project `config/read mcp_servers` 冒充 Task 可读清单。手动重试固定调用 `config/mcpServer/reload` 并返回目标 Task 的新状态页；Task 释放时必须清理对应启动状态。MCP 清单读取或重载失败时，Server 必须将 Codex 原始 `Error.message` 作为结构化 `PROVIDER_ERROR` 返回，Client 必须保留该消息、错误码和 HTTP 状态，禁止只展示通用 `502` 或将消息替换为通用 Provider 错误。
...
- `item/permissions/requestApproval` 的 Turn Scope 授权随当前 Turn 结束，会话 Scope 授权只保留在当前 Codex Session；两者都不得写入 Global、Project 或 Task 持久设置，Client Resolution 只能包含原请求权限的用户选择子集。
...
- Tauri 与 N-API 的交付订阅必须共用 Runtime 内的有界注册表和单一状态机。Runtime 统一负责 live-first 注册、checkpoint、空 `session_id` 解析为当前 session、replay 边界、sequence 过滤、resync、取消、发送失败和 RAII 清理；两个适配器只投递 Runtime 已序列化的最终 frame，不得解析或重建控制帧。Tauri 的投递点是有界 mailbox `admit`，N-API 仍是 callback 发送。同步 N-API 等宿主线程启动订阅时必须传入宿主创建期保存的 `tokio::runtime::Handle`，由 Runtime 使用 `TaskTracker::spawn_on` 调度，禁止依赖调用线程存在 current Tokio runtime。Runtime 关闭时必须先停止接收并取消全部交付订阅；交付层并发关闭统一复用 Runtime `ShutdownGate`，不得各自维护原子状态副本。Desktop 退出前必须 `close_all` mailbox，避免 `admit` 等待者挂起。
...
- Tauri Channel 只发送远小于 8 KiB 的 `event.available` 唤醒，不得推送业务 frame 或使用全局窗口事件；前端通过 `event_pull` 按最多 64 事件 / 256 KiB 拉取 length-prefixed raw Response。Desktop mailbox 最多 64 事件 / 256 KiB，`send` Future 在 `admit` 等待，使 Runtime `subscriber_capacity` 与慢订阅者 resync 对 WebView 生效。N-API 只执行 bridge frame 大小检查、宿主所需的最终字节复制和 callback 发送。
