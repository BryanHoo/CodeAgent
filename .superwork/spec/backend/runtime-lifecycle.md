...
- Codex `0.149.0` 的 `thread/queue/changed`、`thread/reverted`、`project/changed` 与 `thread/project/updated` 尚未进入 CodeAgent 统一产品契约，必须归入 ignored 并在初始化时 opt-out；CodeAgent 继续拥有 Project 边界，不得把实验 Project、Thread Queue、Revert 或 Diagnostics 原始结构直接透传到 Core、Protocol 或 Web。
