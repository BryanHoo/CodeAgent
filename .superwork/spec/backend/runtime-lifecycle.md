...
- 当前 Task 可读取的 MCP 服务只通过 `mcpServerStatus/list { threadId, detail: "toolsAndAuthOnly" }` 分页获取；Provider 必须先确认 Task 归属当前 Project，并只向 Core、Protocol 和 Web 暴露去重后的服务名称，禁止让工具、资源、认证状态、command、args、env、URL 或 Secret 越过 Provider 边界。不得使用无 `threadId` 的进程级结果，也不得回退到 Project `config/read mcp_servers` 冒充 Task 可读清单。
