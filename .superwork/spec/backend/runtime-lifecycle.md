...
- Codex `0.148.0` 的 `thread/queue/changed` 与 `thread/reverted` 尚未进入 CodeAgent 统一产品契约，必须归入 ignored 并在初始化时 opt-out；不得把实验 Thread Queue、Revert 或 Diagnostics 原始结构直接透传到 Core、Protocol 或 Web。
...
- Codex `item/plan/delta`、Reasoning Summary 分段、MCP Progress、File Patch、Turn Diff、Hook、Safety Buffer、Model Reroute、Model Verification、Guardian Warning 与结构化 Error 必须映射为 Provider 无关事件或 Item。高频文本事件按 Item 追加，状态快照按 Item 或 Turn 替换；`item.completed` 和 `turn.completed` 仍是最终权威实体。`contextCompaction` 必须映射为带 `transient` 标记的统一 Activity，使开始、完成与历史 Snapshot 保持同一语义。
