...
- Desktop Tauri `setup` 不得等待 SQLite 目录创建、连接打开、备份、迁移、完整性检查、附件目录初始化或 login shell 探测。SQLite owner thread 必须先返回有界队列句柄，数据库任务通过 Tokio `oneshot` 和有界异步超时返回结果；Repository、File 与 Git 的异步调用链不得使用 `recv`、`recv_timeout` 或其他同步结果等待占用 Tokio worker。首个数据库请求在既有超时内等待初始化结果；附件目录在首次写入时异步初始化；login shell 在后台完成后原子更新 Git、Project Open 与 Codex 共享的同一 `PATH` 快照。
