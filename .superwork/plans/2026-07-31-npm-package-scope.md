...
**Goal:** 将首次发布的 npm 包名更新为 `@bryanhu/codea-gent`，修复标签检出时的发布校验，并完成 GitHub Release 与 npm `0.0.1` 发布。
...
- npm 发布包名必须精确为 `@bryanhu/codea-gent`，版本保持 `0.0.1`。
...
- 根包以 `@bryanhu/codea-gent` 发布，CLI 仍通过 `code-agent` 执行，workflow 不硬编码旧包名。
...
Expected: package verification 通过，manifest 名称为 `@bryanhu/codea-gent` 且 bin 为 `code-agent`。
...
- 所有面向 npm 发布的引用使用 `@bryanhu/codea-gent`，CLI 相关引用继续使用 `code-agent`。
...
Expected: 全部检查通过；`@bryanhu/codea-gent@0.0.1` 和 GitHub Release `v0.0.1` 均可查询。
